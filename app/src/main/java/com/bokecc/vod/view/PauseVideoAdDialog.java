...
public class PauseVideoAdDialog extends Dialog implements TextureView.SurfaceTextureListener, MediaPlayer.OnPreparedListener {
    ...
    private void init() {
        ...
        final ImageView ivVideoAdSound = view.findViewById(R.id.iv_video_ad_sound);
        ivVideoAdSound.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (isVideoAdSoundOn) {
                    player.setVolume(0.0f, 0.0f);
                    ivVideoAdSound.setImageResource(R.mipmap.iv_video_ad_sound_off);
                    isVideoAdSoundOn = false;
                } else {
                    player.setVolume(1.0f, 1.0f);
                    ivVideoAdSound.setImageResource(R.mipmap.iv_video_ad_sound_on);
                    isVideoAdSoundOn = true;
                }
            }
        });

        ImageView ivClosePauseVideoAd = view.findViewById(R.id.iv_close_pause_video_ad);
        ivClosePauseVideoAd.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                dismiss();
            }
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surfaceTexture, int i, int i1) {
        Surface playSurface = new Surface(surfaceTexture);
    ...
    public void updateView(boolean isFullScreen) {
        if (videoHeight > 0 && videoWidth > 0) {
            ...
            ViewGroup.LayoutParams videoParams = tvVideo.getLayoutParams();
            videoParams.width = adVideoWidth;
            videoParams.height = adVideoHeight;
            tvVideo.setLayoutParams(videoParams);
            ...
            if (!isFullScreen) {
                lp.y = (MultiUtils.dipToPx(context, 200) - adVideoHeight) / 2;
                dialogWindow.setGravity(Gravity.TOP);
            } else {
                lp.y = 0;
                dialogWindow.setGravity(Gravity.CENTER);
